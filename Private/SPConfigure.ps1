class SPConfigure {
  ...
  SPConfigure() {
    ...
    [string]$_pathToConfig = ".\Public\Config.psd1"
    Write-Verbose "Setting configuration from Config.psd1"
    $this.Configuration = [PSCustomObject](Import-PowerShellDataFile $_pathToConfig)
    Write-Verbose "Configuration Complete!"
  }

  # Configure with file from custom location
  SPConfigure([string]$_pathToConfig) {
    Write-Verbose "Setting configuration from Config.psd1"
    $this.Configuration = [PSCustomObject](Import-PowerShellDataFile $_pathToConfig)
    Write-Verbose "Configuration Complete!"
  }
  ...
  SPConfigure([PSCustomObject]$config, [string]$_pathToConfig = ".\Public\Config.psd1") {
    $initialConfig = [PSCustomObject](Import-PowerShellDataFile $_pathToConfig)
